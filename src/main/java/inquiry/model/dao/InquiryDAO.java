...
public class InquiryDAO {
	...
	public List<Inquiry> selectInquiryList(SqlSession session, int currentPage) {
		...
		List<Inquiry> nList = session.selectList("InquiryMapper.selectInquiryList", null, rowBounds);
		return nList;
	}
	...
	public int insertInquiry(SqlSession session, Inquiry inquiry) {
		...
		//한줄 쓰고 끝나는게 아니라 inquiry-mapper.xml을 만들고 태그를 이용해서 쿼리문 작성해주고
	...
	public Inquiry selectOneById(SqlSession session, int inquiryNo) {
		Inquiry inquiry = session.selectOne("InquiryMapper.selectOneById", inquiryNo);
		return inquiry;
	}

	public int updateInquiry(SqlSession session, Inquiry inquiry) {
		int result = session.insert("InquiryMapper.updateInquiry", inquiry);
		return result;
	}

	public int deleteInquiryByNo(SqlSession session, int inquiryNo) {
		int result = session.delete("InquiryMapper.deleteInquiryByNo", inquiryNo);
		return result;
	}
	...
	public String generatePageNavi(SqlSession session, int currentPage) {
		...
		if(startNavi != 1) {
			result.append("<a href='/inquiry/ilist.do?currentPage="+(startNavi-1)+"'>[이전]</a>&nbsp;&nbsp;");
		}
		
		for(int i = startNavi; i <= endNavi; i++) {
			result.append("<a href='/inquiry/ilist.do?currentPage="+i+"'>"+i+"</a>&nbsp;&nbsp;");
		}
		
		if(startNavi != totalNaviCount) {
			result.append("<a href='/inquiry/ilist.do?currentPage="+(endNavi+1)+"'>[다음]</a>&nbsp;&nbsp;");
		}
